...
class App {
  constructor(canvas) {
    ...
    Quoridor().then(instance => {
      console.log("Quoridor WASM loaded successfully.");
      ...
      this.quoridor.createGame(this.boardDimension).then(() => {
          console.log("Game created. Starting...");
          this.startGame();
      }).catch(e => console.error("Error creating game instance:", e));
    }).catch(e => {
        console.error("FAILED to load Quoridor WASM:", e);
    });
  }
  ...
  async updatePrediction() {
    ...
    // In Scene.js, walls are stored in this.players[i].fences
    // We check if they are "on the board" by comparing x/y to the initial off-board values
    const walls = [];
    const boardOffset = this.boardOffset; // 2.0 for a 5x5 board

    this.players.forEach(player => {
      player.fences.forEach(fence => {
        // Check if fence has been moved from its initial off-board spawn
        // Initial off-board X is roughly +/- (boardOffset + 1.5)
        if (Math.abs(fence.x) < boardOffset + 1.0) {
           const nx = Math.round(fence.x + boardOffset - 0.5);
           const ny = Math.round(fence.y + boardOffset - 0.5);
           // Rotation 0 is horizontal, 90 is vertical
           const isHorizontal = (Math.abs(fence.rotation) < 1.0) ? 1 : 0; 
           walls.push([nx, ny, isHorizontal]);
        }
      });
    });

    const payload = {
        p1_pos: [pawnPositions.whiteNx, pawnPositions.whiteNy],
        p2_pos: [pawnPositions.blackNx, pawnPositions.blackNy],
        walls: walls,
        turn: this.current + 1 // 1 or 2
    };

    try {
      // --- 2. FETCH MOVE FROM PYTHON ---
      ...
      console.log("AI Move:", moveData);

      // --- 3. TRANSLATE TO GAME ACTION ---
      let actionToExecute = -1;

      if (moveData.action_type === "move") {
          const validMoves = this.quoridor.validActionMovements();
          const player = this.players[this.current];
          const opponent = this.players[1 - this.current];
          const [tX, tY] = moveData.coordinates;

          // Iterate all valid moves to find the one that matches target [tX, tY]
          for (let i = 0; i < validMoves.size(); i++) {
              const am = validMoves.get(i);
              // Calculate resulting visual coordinates from this action
              let vx = player.pawn.x + am.movement.dx;
              let vy = player.pawn.y + am.movement.dy;
              
              // Handle jump logic (copied from showActions)
              if ((Math.abs(vx - opponent.pawn.x) < 1e-6) && 
                  (Math.abs(vy - opponent.pawn.y) < 1e-6)) {
                  vx += am.movement.dx;
                  vy += am.movement.dy;
              }
              
              // Convert visual back to grid to compare with Python target
              if (Math.round(vx + boardOffset) === tX && Math.round(vy + boardOffset) === tY) {
                  actionToExecute = am.action;
                  break;
              }
          }
      } else if (moveData.action_type === "wall") {
          const validPlacements = this.quoridor.validActionPlacements();
          const [tX, tY] = moveData.coordinates;
          const isHoriz = (moveData.orientation === "horizontal");
          
          for (let i = 0; i < validPlacements.size(); i++) {
              const ap = validPlacements.get(i);
              if (ap.placement.nx === tX && ap.placement.ny === tY && ap.placement.horizontal === isHoriz) {
                  actionToExecute = ap.action;
                  break;
              }
          }
      }
      ...
      if (actionToExecute !== -1) {
          ...
          console.error("AI attempted illegal move:", moveData);
      }
